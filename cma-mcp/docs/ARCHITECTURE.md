...
see [`MCP_SERVER.md`](MCP_SERVER.md). For the project's reasoning
about its own design, see [`ANTICIPATED_CRITIQUES.md`](ANTICIPATED_CRITIQUES.md).
For the longer-arc strategy, see [`STRATEGY.md`](../../STRATEGY.md)
and [`DECISIONS.md`](../../DECISIONS.md) at the repository root.
...
The contribution is **reach**, not new capability. STRATEGY DD-1
locks subprocess-over-reimplementation; DECISIONS AD-008 locks
same-repo-as-cma; DECISIONS AD-001 locks no-MCP-SDK-dependency.
...
the right balance (see ANTICIPATED_CRITIQUES C-9).
...
Test suite (`tests/`): 36 cases plus the wheel-install smoke step
in CI.
